...
internal sealed partial class MainWindow : Window
{
	...
	public MainWindow()
	{
		...
		// Keep the whole UI visible on smaller Mac displays (including the Dock/menu bar).
		// Avalonia reports the working area in physical pixels, so convert it to DIPs.
		...
		_midiPickerType = new FilePickerFileType("MIDI Files")
		{
			...
			// macOS uses Uniform Type Identifiers for the native file picker.
			// Without this, the old Avalonia preview can show unrelated files.
	...
	private void FitToWorkingArea()
	{
		...
		// Leave a small margin so the window never ends up underneath the Dock.
		Width = Math.Min(1400, Math.Max(1100, workingWidth - 24));
		Height = Math.Min(800, Math.Max(650, workingHeight - 24));
